...
class ModelConfig:
    facts: int = 4
    num_body_literals: int = 2
    num_pos_body_literals: int = 1
...
class Atom:
    ...
    def is_ground(self) -> bool:
        return all(not arg[0].islower() for arg in self.args)
...
class OpenAIClient(LLM):
    ...
    def send_prompt(self, prompt: str, model="gpt-5-mini") -> str:
...
class OllamaClient(LLM):
    ...
    def send_prompt(self, prompt: str, model="gpt-oss:20b") -> str:
...
def generate_example(config: ModelConfig) -> KnowledgeBase:
    ...
    for desc_id in range(config.max_predicates):
        predicates.add(f"d{desc_id}")

        for obj_id in range(config.num_body_literals):
            possible_atoms.add(Atom(f"d{desc_id}", (f"o{obj_id}", )))

    # Generate instance
    facts = set(random.sample(list(possible_atoms), config.facts))
    ...
    for rule_id in range(config.num_rules):
        ...
        body = set()
        max_non_var = config.max_body_atoms - 1
        for literal_id in range(max_non_var):

            # Select an unused atom
            to_ground_pred = random.sample(remaining_pred, 1)[0]
            atom = Atom(to_ground_pred, "X")

            # Ensure at least one positive and possibly allow for negative
            pos = rule_id < config.num_pos_body_literals
...
def run_asp(program: str) -> Dict:
    ...
    # nested dictionary of solver internals
    stats = ctl.statistics  

    # Useful top-level stats (may vary by solver configuration)
    solver_stats = {
...
def generate_benchmarks():
    ...
    shutil.rmtree("orig_benchmarks")
    shutil.rmtree("orig_solutions")

    for index in range(200):
        for negation_odds in [0, 1]:

            kb = generate_example(config, negation_odds)
            model_stats = run_asp(str(kb))

            # Check for no more or less than one stable model
            if len(model_stats["witnesses"]) != 1:
                raise RuntimeError(f"Too many stable models: {index}")

            atoms = model_stats["witnesses"][0]['atoms']
            warnings = model_stats["warnings"]

            warnings_folder = "warn" if len(warnings) > 0 else "exact"
            negation_folder = "negation" if negation_odds == 1 else "no_negation"
            atoms_folder = f"atoms_{len(atoms)}"
            deep_path = f"{warnings_folder}/{negation_folder}/{atoms_folder}"

            benchmark_path = f"orig_benchmarks/{deep_path}/benchmark_{index}.lp"
            solution_path = f"orig_solutions/{deep_path}/solution_{index}.json"
...
def run_llm_for_rules(client: LLM, original_kb: KnowledgeBase, original_stats: Dict, response_path: str, rerun=False) -> KnowledgeBase:
    ...
    if not os.path.exists(response_path) or rerun:
        ...
        response = Path(response_path).read_text()

    lines = response.split("\n")
    new_rules = []

    for line_num in range(len(lines)-1, -1, -1):
        rule_str = lines[line_num]
        try:
...
def rerun_clingo_loop():

    info_path = "exact/negation/atoms_6"

    orig_benchmark_path = f"orig_benchmarks/{info_path}/benchmark_114.lp"
    orig_solution_path = f"orig_solutions/{info_path}/solution_114.json"

    llm_responses_path = f"llm_responses/gpt_5_mini/{info_path}/response_114_0.txt"
    llm_benchmarks_path = f"llm_benchmarks/{info_path}/benchmark_114_0.lp"
    llm_solutions_path = f"llm_solutions/{info_path}/solution_114_0.json"
    ...
    # Run LLM
    llm_kb = run_llm_for_rules(client, original_kb, original_stats, llm_responses_path, True)
