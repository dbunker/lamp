...
\begin{tikzpicture}[remember picture,overlay]
  \fill[potsdamblue]
    ([xshift=1.20in,yshift=-1.05in]current page.north west)
    rectangle
    ([xshift=24.0in,yshift=-6.85in]current page.north west);
  ...
  \node[
    ...
    text width=22.0in,
    align=left
  ] at ([xshift=1.55in,yshift=-1.55in]current page.north west) {%
    {\bfseries\fontsize{50}{54}\selectfont
  ...
  \node[
    ...
    text=white,
    align=left
  ] at ([xshift=1.58in,yshift=-4.35in]current page.north west) {%
    {\bfseries\fontsize{30}{34}\selectfont David Bunker}\\[0.10in]
    {\fontsize{24}{28}\selectfont University of Potsdam}
  };
  ...
  \node[
    ...
    text width=31.5in,
    align=left
  ] at ([xshift=1.35in,yshift=0.92in]current page.south west) {%
    {\bfseries\fontsize{18}{20}\selectfont Take-home:}
    {\fontsize{18}{20}\selectfont
    Solver-in-the-loop prompting makes ASP rule induction practical: ILASP remains the fast symbolic baseline, while LAMP offers a complementary LLM workflow that can sometimes avoid the search bottlenecks of exhaustive methods.}
  };
\end{tikzpicture}
...
\vspace{0.08in}
\tinybody
Source code, benchmark data, and generated ILASP tasks are available at \url{https://github.com/dbunker/lamp}.
\end{posterbox}
\end{minipage}
\end{minipage}
\end{center}
