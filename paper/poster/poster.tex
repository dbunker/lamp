...
\item \textbf{Hybrid direction:} LAMP is not a replacement for ILASP; it is a complementary LLM-guided search strategy.
\end{tightitems}
\end{posterbox}

\sectiongap

\begin{posterbox}{Scope And Next Steps}
\begin{tightitems}
...
\item Next: richer ASP constructs, ASPBench or planning domains, and ILASP-style search with an agentic LLM refinement loop.
\end{tightitems}
\end{posterbox}
\end{minipage}
...
\begin{tightitems}
\item Same input instance for both systems: facts $F$, target answer set $A^*$, and bounded rule schema.
\item LAMP receives $A^*$ in the prompt; ILASP receives it as a context-dependent positive example.
\item Correctness is exact \textsc{Clingo} stable-model matching, so candidates are not accepted by surface similarity.
\end{tightitems}
\end{minipage}\hfill
\begin{minipage}[t]{0.315\linewidth}
\smallbody
\textbf{Run conditions}
\begin{tightitems}
\item 120 synthetic benchmarks over unary normal rules, with all systems limited to the same 420\,s timeout.
\item LAMP used 3 attempts: one initial generation and up to 2 solver-feedback refinements.
\item \textit{gpt-5-mini} used the OpenAI API; \textit{gpt-oss:20b} and \textit{qwen3-coder:30b} ran locally with Ollama.
\end{tightitems}
\end{minipage}\hfill
\begin{minipage}[t]{0.315\linewidth}
\smallbody
\textbf{Timing and data}
\begin{tightitems}
\item ILASP failures are recorded at the timeout ceiling; \textit{gpt-5-mini} timing excludes network latency.
\item Output programs are judged by their produced stable model, literal count, and stratification rather than source-rule identity.
\item Source code, benchmark data, and generated ILASP tasks are available with the paper artifacts.
\end{tightitems}
\vspace{0.04in}
\tinybody
\url{https://github.com/dbunker/lamp}
\end{minipage}
\end{posterbox}
\end{minipage}
\end{center}
