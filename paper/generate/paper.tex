...
\begin{itemize}
  ...
  \item $\Phi$ is the \emph{output format schema}, a set of at most $R$ ASP rules, each of the form
    \[
      d_h(X) \;:\text{-}\; p_1(X),\; \ldots,\; p_{k}(X),\; \texttt{not}\; n_1(X),\; \ldots,\; \texttt{not}\; n_{m}(X).
    \]
    where $d_h,\, p_i,\, n_j \in \mathcal{D}$, $X$ is a single universally-quantified variable, $k \leq L^+$ positive body literals, and $m \leq L^-$ negative body literals.
\end{itemize}
A \emph{candidate program} $\Pi$ is a finite set of rules consistent with $\Phi$.
...
A \emph{LAMP Interaction Sequence} for task $\mathcal{M} = \langle F, A^{*}, \mathcal{S}, \Phi \rangle$
...
Given a LAMP Modeling Task $\mathcal{M} = \langle F, A^{*}, \mathcal{S}, \Phi \rangle$
