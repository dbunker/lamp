...
\begin{definition}[ASP Configuration Schema]
An \emph{ASP Configuration Schema} is a tuple
\[
  \mathcal{S} \;=\; \langle\, \mathcal{D},\; \mathcal{T},\; \mathcal{P} \,\rangle
\]
where $\mathcal{D}$ is a finite set of descriptor predicates of arity~$1$,
$\mathcal{T}$ is a finite set of object terms, and
$\mathcal{P} \subseteq \mathcal{D} \times \mathcal{T}$ is the set of admissible predicate--term pairs.
A \emph{fact} over $\mathcal{S}$ is any ground atom $d(t)$ with $d \in \mathcal{D}$ and $t \in \mathcal{T}$.
A \emph{normal rule} over $\mathcal{S}$ is a rule of the form
\[
  d_0(X) \mathbin{:-} d_1(X),\,\ldots,\,d_k(X),\;
             \mathbf{not}\; d_{k+1}(X),\,\ldots,\,\mathbf{not}\; d_n(X)
\]
where $d_0,\ldots,d_n \in \mathcal{D}$, $X$ is a universally quantified variable,
$k \geq 1$ positive body literals precede all negative ones, and
$d_0 \notin \{d_1,\ldots,d_k\}$ (the head is not trivially entailed by the positive body alone).
\end{definition}
...
\begin{figure}[h!]
  ...
  \begin{tikzpicture}[node distance=0.55cm and 1.6cm]
    ...
    \node[decision, below=0.8cm of clingonode]
      (check) {\textbf{Evaluate for Optimality}};
...
\begin{definition}[LAMP Modeling Task]
A \emph{LAMP Modeling Task} is a tuple
\[
  \mathcal{M} \;=\; \langle\, F,\; A^{*},\; \mathcal{S},\; \Phi \,\rangle
\]
where:
\begin{itemize}
  \item $F$ is a finite set of ground ASP facts over schema $\mathcal{S}$, called the \emph{instance facts};
  \item $A^{*} \subseteq \mathit{HB}_{\mathcal{S}}$ is the \emph{expected stable model}, a finite set of ground atoms over the Herbrand base of $\mathcal{S}$ such that $F \subseteq A^{*}$ and $A^{*} \setminus F \neq \emptyset$ (at least one atom must be newly derived);
  \item $\mathcal{S} = \langle \mathcal{D}, \mathcal{T}, \mathcal{P} \rangle$ is the ASP Configuration Schema (Definition~1) defining the admissible vocabulary; and
  \item $\Phi$ is an \emph{output format constraint} specifying the syntactic form of candidate rules (e.g., arity-$1$ predicates, a single universally quantified variable, bounded numbers of positive and negative body literals).
\end{itemize}
A \emph{candidate program} $\Pi$ is a finite set of normal rules over $\mathcal{S}$ consistent with $\Phi$.
