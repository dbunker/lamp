...
An \emph{instance} of a complexity category $\mathcal{C}$ as input into LAMP is a tuple
\[
  \mathcal{M} \;=\; \langle\, F,\; A^{*},\; \Phi \,\rangle
\]
where:
\begin{itemize}
  \item $F$ is the \emph{instance facts}, a set of ground atoms with any descriptor over any object (e.g.\ \texttt{d0(o0)});
  ...
  \item $\Phi$ is the \emph{output format}, a set of at most $R$ rules with ungrounded atoms of the form
    \[
      \texttt{d\{m\}(X) :- [not] d\{a\}(X), \ldots}
    \]
    with at most $L^+$ positive and $L^-$ negative body literals per rule.
\end{itemize}
